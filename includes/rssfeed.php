...
</select><small><?php echo $value['description']; ?></small><div class="clearfix"></div>
</div>
<?php 
break ;
case "textarea":
...
<div class="form-group">
	<label for="<?php echo $value['id']; ?>" class="col-sm-3 control-label"><?php echo $value['name']; ?></label>
 	<div class="col-sm-9"><textarea name="<?php echo $value['id']; ?>" type="<?php echo $value['type']; ?>" cols="" rows=""><?php if ( get_option( $value['id'] ) != "") { echo stripslashes(get_option( $value['id']) ); } else { echo $value['std']; } ?></textarea>
 	<small><?php echo $value['description']; ?></small></div><div class="clearfix"></div>
 </div>
    <?php		}
	}
?>
